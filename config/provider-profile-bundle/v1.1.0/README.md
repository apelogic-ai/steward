# Steward runtime-provider profile bundle 1.1.0

This immutable bundle supersedes `steward-runtime-providers@1.0.0`. It adds the
pinned arm64 and amd64 Codex executables to both runtime provider profiles. The
portable endpoint, authorization, capability, and runtime-grant contracts are
otherwise unchanged.

The bundle remains deployment-neutral. A deployment supplies only the exact
HTTPS origins and canonical service CIDRs declared in `bundle.json`; it cannot
supply credentials or policy-bearing fields.
...
```json
{
  ...
  "bundle": {"id": "steward-runtime-providers", "version": "1.1.0"},
  "profiles": [
    {
      "id": "steward-mcp-gw",
      "inputs": {
...
Keep the original 1.0.0 input document and create a 1.1.0 input document with
...
cargo xtask provider-profile-bundle upgrade \
  --from-inputs inputs-1.0.0.json \
  --inputs inputs-1.1.0.json \
  --output rendered-profiles
```

The command first renders 1.0.0 from its frozen source and requires the current
installation to match it exactly. It rejects drift, any unknown predecessor,
and any destination other than the declared 1.0.0 to 1.1.0 transition. It
stages and validates the complete replacement before swapping directories,
restores the predecessor if publication fails, and verifies 1.1.0 after the
swap. Restart the consumer only after the command succeeds.

Fresh installations use the ordinary exact-version path:

```sh
cargo xtask provider-profile-bundle install --inputs inputs-1.1.0.json --output rendered-profiles
cargo xtask provider-profile-bundle reconcile --inputs inputs-1.1.0.json --output rendered-profiles
```
...
that `bundle.json` identifies `steward-runtime-providers@1.1.0`, validate the
manifest and templates, and run the product-owned renderer, installer, or
upgrade command. The source-only validation does not make this bundle eligible for release. The
release workflow also requires the authenticated OpenShell adapter E2E lane on
its native linux/amd64 runner.
