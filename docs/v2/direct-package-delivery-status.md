...
| DP-R01 | `apelogic-ai/steward-run` | pin correction | PR `#31` merged as `fd090be`, but the self-hosted workflow still pins a legacy-only nested action; an isolated immutable-pin correction is required. |
...
| DP-R01 | DP-O01 | merged source `fd090be`, implementation `79b9f42`, contract checkpoint `042e8aa`; nested action remains pinned to legacy-only `0707623` | not yet a usable caller pin |
