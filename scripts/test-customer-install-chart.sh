...
if ! grep -Eq '^version: (0\.1\.23|0\.2\.0)$' "${root}/charts/steward/Chart.yaml"; then
  echo 'customer install contract supports only the v0.1.23 transition base or v0.2.0' >&2
  exit 1
fi
