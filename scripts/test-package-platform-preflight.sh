...
tar -xzf "$temporary/provider-release/steward-runtime-providers-0.2.6.tar.gz" -C "$temporary"
provider_bundle="$temporary/provider-profile-bundle/v1.2.0"
for name in first second; do
  "$root/scripts/package-platform-preflight.sh" 0.2.6 "$temporary/$name"
done
first="$temporary/first/steward-platform-preflight-0.2.6.tar.gz"
second="$temporary/second/steward-platform-preflight-0.2.6.tar.gz"
cmp "$first" "$second"
