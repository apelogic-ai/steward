...
- `config.controller.openshellTaskLogMode` defaults to `off`. Set it to `full`
  to mirror redacted task-process stdout and stderr event metadata into
  controller logs while the task runs; each record includes `runtime_uid`,
  `workspace`, `sandbox`, `stream`, and `bytes`. Task-controlled output is never
  copied into controller logs because it may contain credentials.
